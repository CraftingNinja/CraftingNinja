...
class ManualData
{
	...
	{
        $this->path = $this->service->dataDir . 'manual/';
	}
	...
	public function nodeCoordinates()
	{
		...
		$areaFinder = collect($this->service->data['location'])->pluck('name', 'id');
		...
		foreach ($this->service->data['node'] as &$node) {
	...
	public function nodeTimers()
	{
		...
		foreach ($nodeTimers as $nodeId => $timer)
			if (isset($this->service->data['node'][$nodeId]))
				$this->service->data['node'][$nodeId]['timer'] = $timer;
	}
	...
	public function randomVentureItems()
	{
		...
		$ventures = collect($this->service->data['venture'])->pluck('id', 'name')->toArray();
		$items = collect($this->service->data['item'])->pluck('id', 'name')->toArray();
	...
	public function leveTypes()
	{
		...
		foreach ($this->service->data['leve'] as &$leve)
