...
class Item extends FFXIVModel {

	protected $table = 'item';

	public function fishing()
	{
		return $this->belongsToMany(Fishing::class);
	}
	...
	{
		return $this->belongsToMany(Quest::class, 'quest_reward')->withPivot('amount');
	}

	public function quest_required()
	{
		return $this->belongsToMany(Quest::class, 'quest_required');
	}

	public function leve_rewards()
	{
		return $this->belongsToMany(Leve::class, 'leve_reward')->withPivot('amount', 'rate');
	}

	public function leve_required()
	{
		return $this->belongsToMany(Leve::class, 'leve_required')->withPivot('amount');
	}
	...
	{
		return $this->belongsToMany(Recipe::class, 'recipe_reagents')->withPivot('amount');
	}
	...
	public static function localized_name_variable()
	{
		return [
			...
			'de' => 'de_',
			'fr' => 'fr_',
        // ][config('language')] . 'name'; // TODO - Language?
        ]['en'] . 'name';
	}

	public static function calculate($job_id = 0, $level = 1, $range = 0, $craftable_only = TRUE, $rewardable_too = TRUE)
	{
		// $cache_key = __METHOD__ . '|' . config('language') . '|' . $job_id . ',' . $level . ',' . $range . ($craftable_only ? ('T' . ($rewardable_too ? 'T' : 'F')) : 'F');
		...
		foreach (range($level, $level + $range) as $l)
		{
			...
			foreach ($equipment_list as $role => $items)
			{
				...
				foreach ($items as $key => $item)
					...
					{
							//$item->cannot_equip = array_flip(config('site.defined_slots'))['Off Hand'];
		...
		// Cache the results
		// Cache::put($cache_key, $leveled_equipment, config('site.cache_length'));
